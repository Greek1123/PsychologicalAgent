...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate a demo acceptance checklist.")
    parser.add_argument("--out", default=str(ROOT / "docs" / "demo_acceptance_checklist.md"))
    args = parser.parse_args()

    out = Path(args.out)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(build_checklist(ROOT), encoding="utf-8")
    print(f"output = {out}")
    return 0
