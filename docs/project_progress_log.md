...
# 2026-06-05 随机 30 条真实回复抽检后处理层修复

## 本次做了什么

- 继续基于随机 30 条真实回复人工抽检结果优化后端处理层，而不是单纯追求自动分数。
- 扩展 `followup_reply_overrides` 的上下文推进规则，覆盖并收紧：
  - 考试前“很堵/不知道怎么说”的弱输出，不再误走危机或其他主题。
  - 实习面试“只能做十分钟”的启动阻滞，直接给可写下来的复盘骨架。
  - 专业不喜欢/转专业不确定的“很空/十分钟/一小步”，不再被面试或任务模板抢走。
  - 宠物离世后看到照片受不了，必须有宠物离世上下文才触发，避免普通“拍照确认”误触发。
  - 暴食后的“一小步”推进到正常进食和打断节食循环，不重复首轮羞耻安抚。
  - 被模仿说话/欺凌触发后，下次再发生时给边界句和离场路径。
  - 疑似被监控、被盯着、连续失眠场景，弱输出继续保持医疗/校医院评估方向，同时收紧关键词，避免“说我”这种宽词误触发。
- 同步调整 `scripts/generate_random_reply_audit.py` 的额外追问生成规则，优先识别宠物离世、专业迷茫、班群社死、欺凌触发、进食/外貌等具体场景，再进入泛化问题模板。
- 新增回归测试，专门覆盖专业、宠物、暴食小步、欺凌下次回应和疑似被监控不被考试模板抢走。
...
python scripts\generate_random_reply_audit.py --sample-size 30 --seed 20260603
markdown = reports/random_reply_audits/20260605_222227_random_reply_audit.md
json = reports/random_reply_audits/20260605_222227_random_reply_audit.json
```
...
- 继续按成熟项目推进，没有生成新的随机报告。
- 新增 `src/campus_support_agent/privacy_redaction.py`，统一识别手机号、邮箱、证件号、学号/工号、微信和 QQ，并返回脱敏统计。
- `student` 视图保留学生端必要字段，同时对可见 `reply_text/latest_reply_text` 做直接标识脱敏。
- `counselor` 视图保留风险、状态、照护、处理链路等专业上下文，但移除后端内部字段，并遮蔽直接身份标识。
- `research` 视图继续移除自由文本、人工干预备注和会话记忆，同时附带 `privacy_redaction` 统计，便于研究面板证明数据已降敏。
- `admin` 视图保留原始数据，用于授权审计和本地排障。
- `/api/v1/frontend/contract` 的字段说明同步补充 `privacy_redaction` 和角色视图边界。
